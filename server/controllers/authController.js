...
// importando módulos necessários para a autenticação:
...
const {
    ...
    generateResetToken,
    sendWelcomeEmail, // Nova função para enviar o email de boas-vindas
...
const registerControll = async (req, res) => {
    ...
    try {
        ...
        if (existingUser.rows.length > 0) {
            ...
            const addUser = await connection.query("INSERT INTO users (name, last_name, email, password, admin) VALUES ($1,$2,$3,$4,$5)", [name, lastName, email, hashPassword, admin]);
  ...
  try {
      ...
      if (existingUser.rows.length === 0) {
          ...
          if (user.first_pass) {
            ...
            // Atualiza a senha e desativa a flag de primeira senha
          ...
          const expiresIN = "3h";
          const token = jwt.sign({name: user.name + " " + user.last_name, email: user.email}, process.env.SECRET, {
              expiresIn: expiresIN,
          });
...
module.exports = {registerControll, loginControll, verifyToken, forgotPasswordControll, resetPasswordPOST, logoutControll};